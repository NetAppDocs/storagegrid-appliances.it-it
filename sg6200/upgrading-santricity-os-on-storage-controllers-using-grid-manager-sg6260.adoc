...
summary: 'Per i controller di storage che utilizzano attualmente SANtricity OS 08.42.20.00 (11.42) o versioni successive, puoi usare Grid Manager o la modalità di manutenzione per applicare un aggiornamento.' 
---
= Aggiorna il sistema operativo SANtricity sui controller di storage SG6200 utilizzando Grid Manager
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
È possibile utilizzare Grid Manager o la modalità di manutenzione per applicare un aggiornamento.
...
La procedura non sarà completata finché l'aggiornamento del sistema operativo SANtricity non sarà stato applicato correttamente a tutti i nodi applicabili selezionati per l'aggiornamento.  Potrebbero essere necessari più di 30 minuti per caricare il sistema operativo SANtricity su ciascun nodo (in sequenza) e fino a 90 minuti per riavviare ciascun dispositivo di archiviazione StorageGRID .  I nodi nella griglia che non utilizzano SANtricity OS non saranno interessati da questa procedura.
...
NOTE: Prima di iniziare questa procedura, applicare l'ultimo hotfix StorageGRID . Vedere https://docs.netapp.com/us-en/storagegrid/maintain/storagegrid-hotfix-procedure.html["Procedura di hotfix StorageGRID"^] per i dettagli.
...
+
Scegli la versione del sistema operativo SANtricity per i tuoi controller di storage.
...
** Selezionare uno o più pulsanti *Approva* per aggiungere uno o più nodi alla coda di aggiornamento del sistema operativo SANtricity. Il pulsante *Approva* è disabilitato se lo stato non è Nominale.
+
...
Il nodo non dispone di un controller di storage che può essere gestito dal sistema StorageGRID. Completare il processo di aggiornamento senza aggiornare il nodo che visualizza questo messaggio.
 ...
 a| 
Il file SANtricity OS non è compatibile con questo nodo.
 a| 
...
. Ripetere questa procedura di aggiornamento per i nodi con una fase Completata che richiedono un file di aggiornamento del sistema operativo SANtricity diverso.
+

NOTE: Per i nodi con stato "Necessita attenzione", utilizzare la modalità di manutenzione per eseguire l'aggiornamento.



.Informazioni correlate
* https://mysupport.netapp.com/matrix["Tool di matrice di interoperabilità NetApp"^]
* link:upgrading-santricity-os-on-storage-controllers-using-maintenance-mode-sg6260.html["Aggiorna il sistema operativo SANtricity sui controller SG6200 utilizzando la modalità di manutenzione"]
