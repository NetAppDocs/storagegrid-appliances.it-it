...
summary: Per aumentare la capacità di archiviazione, è possibile aggiungere uno o due shelf di espansione a un SG6260 implementato in un sistema StorageGRID. 
---
= Aggiungi shelf di espansione a SG6260 già implementato
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Per aumentare la capacità di archiviazione, è possibile aggiungere uno o due shelf di espansione a un SG6260 già implementato in un sistema StorageGRID.
...
* Mettere l'SG6260 in modalità di manutenzione.
...
Il completamento della procedura per uno o due shelf di espansione dovrebbe richiedere un'ora o meno per nodo dell'appliance. Per ridurre al minimo i tempi di inattività, i passaggi seguenti indicano di installare i nuovi shelf di espansione e le relative unità prima di mettere SG6260 in modalità di manutenzione. I passaggi rimanenti dovrebbero richiedere circa 20-30 minuti per nodo dell'appliance.

.Prima di iniziare
* È necessario disporre della passphrase di provisioning.
* È necessario utilizzare StorageGRID 12.0 o versioni successive.
* Si dispone dello shelf di espansione e di due cavi SAS per ogni shelf di espansione.
* Hai link:locating-sg6200-in-data-center.html["il controller si trova fisicamente"] dove stai aggiungendo lo shelf di espansione nel data center.


.Fasi
. Segui le istruzioni per link:../installconfig/sg6260-installing-60-drive-shelves-into-cabinet-or-rack.html["installazione di shelf da 60 dischi in un cabinet o rack"].
. Dal Grid Manager, link:../commonhardware/placing-appliance-into-maintenance-mode.html["Mettere il controller SG6200-CN in modalità di manutenzione"].
...
+
image::../media/expansion_shelves_connections_sg6260.png[Connessioni shelf di espansione SG6260]
...
+
Esempi di messaggi che potrebbero apparire nella pagina iniziale, a seconda del numero di shelf di espansione esistenti o nuovi, come segue:

+
** Un banner visualizzato nella parte superiore della pagina indica il numero totale di ripiani di espansione rilevati.
+
...
** Un messaggio in fondo alla pagina indica che un'espansione è pronta per essere avviata.
+
...
+
Al termine del riavvio, viene visualizzata la scheda *Attività* con le opzioni per riavviare il nodo o mettere l'appliance in modalità di manutenzione.

. Verificare lo stato del nodo di storage dell'appliance e dei nuovi shelf di espansione.
+
.. In Grid Manager, selezionare *Nodi* e verificare che il nodo di archiviazione dell'appliance abbia un'icona con un segno di spunta verde.
+
