...
---
= Come funziona la clonazione dei nodi dell'appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Cloning già in corso
* Configurazione di rete errata
* L'appliance di destinazione non riesce a raggiungere il nodo amministrativo primario
* Mancanza di connettività tra le appliance di origine e di destinazione
* Gli apparecchi di origine e di destinazione non sono compatibili
* Gli appliance di origine e di destinazione sono in stati di modalità di manutenzione diversi
* Le versioni di StorageGRID Appliance Installer sugli appliance di origine e di destinazione non corrispondono
* Per i nodi di archiviazione, l'appliance di destinazione ha uno storage incompatibile o capacità insufficiente
* Il dispositivo di destinazione non è pronto
...
** Il numero di volumi in un dispositivo di storage di destinazione deve essere maggiore o uguale al numero di volumi nel nodo di origine. L'SGF6112 ha 12 volumi e l'SGF6212 ha 13 volumi. Tutti gli altri dispositivi di storage hanno 16 volumi.
...
+
È possibile visualizzare le informazioni sulla modalità RAID corrente del nodo dell'appliance di origine nella pagina *Nodi* di Grid Manager. Selezionare la scheda *Archiviazione* per l'appliance.

*** Non è possibile clonare un nodo di origine con 16 volumi di archivio di oggetti (rangedb) su un'appliance di storage di destinazione con un numero inferiore di volumi di archivio di oggetti, anche se l'appliance di destinazione ha una capacità maggiore rispetto al nodo di origine.
...
La clonazione dei nodi non è supportata da un'appliance del nodo sorgente SG110 senza crittografia dei nodi a un'appliance di destinazione SG110 con crittografia dei nodi.



[[sg120-appliance-cloning-information-and-restrictions]]
=== Informazioni e restrizioni sulla clonazione del dispositivo SG120

È possibile clonare un'appliance nodo sorgente SG120 su un'appliance di servizi SG1200 di destinazione per conferire maggiori funzionalità al nodo Admin o al nodo Gateway.

La clonazione dei nodi non è supportata da un'appliance SG120 di origine senza crittografia dei nodi a un'appliance SG120 di destinazione con crittografia dei nodi.
...
La clonazione dei nodi non è supportata da un'appliance del nodo sorgente SG1100 senza crittografia dei nodi a un'appliance di destinazione SG1100 con crittografia dei nodi.



[[sg1200-appliance-cloning-information-and-restrictions]]
=== Informazioni e restrizioni sulla clonazione dell'appliance SG1200

È possibile clonare un'appliance nodo sorgente SG1200 su un'appliance di destinazione servizi SG120.

* La clonazione dell'SG1200 su un'appliance di destinazione per servizi SG120 consente di ridistribuire l'SG1200 per un'applicazione più esigente. Ad esempio, se un'appliance SG1200 source node viene utilizzata come Admin Node e si desidera utilizzarla come nodo dedicato per il bilanciamento del carico.
* La sostituzione di un'appliance nodo sorgente SG1200 con un'appliance target di servizi SG120 riduce la velocità massima delle porte di rete da 100-GbE a 25-GbE.


La clonazione dei nodi non è supportata da un'appliance SG1200 di origine senza crittografia dei nodi a un'appliance SG1200 di destinazione con crittografia dei nodi.
...
* Un'appliance nodo sorgente SG5760 configurata con DDP o DDP-16 _non_ può essere clonata su un'appliance di destinazione storage SG6160 o SG6260 con una modalità RAID corrispondente.
...
* È possibile clonare un'appliance nodo sorgente SG6060 senza unità di espansione su un'appliance di destinazione storage SG6160 o SG6260 anch'essa senza unità di espansione, a condizione che le dimensioni delle unità e le modalità RAID siano identiche.
* Un'appliance nodo sorgente SG6060 con qualsiasi numero di unità di espansione _non_ può essere clonata su un'appliance di destinazione storage SG6160 o SG6260, anche se il numero di unità di espansione, le dimensioni delle unità e le modalità RAID sono le stesse.




[[sgf6112-appliance-cloning-information-and-restrictions]]
=== Informazioni e restrizioni sulla clonazione dell'appliance SGF6112

La clonazione dei nodi non è supportata da un'appliance di origine SGF6112 senza crittografia dei nodi a un'appliance di destinazione SGF6112 o SGF6212 con crittografia dei nodi.



[[sg6160-and-sg6260-appliance-cloning-information-and-restrictions]]
=== Informazioni e restrizioni sulla clonazione degli appliance SG6160 e SG6260
