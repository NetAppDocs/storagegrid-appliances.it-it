...
---
= Requisiti per la clonazione del nodo dell'appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Se abiliti il DHCP su qualsiasi porta del nodo di destinazione che era configurata come statica sul nodo di origine, assicurati che non ci siano lease DHCP permanenti per nessuno degli indirizzi MAC del nodo di origine. Se non lo fai, potrebbe verificarsi un conflitto di indirizzi IP al termine del processo di cloning.
...
+
Il nodo sorgente e l'appliance di destinazione possono avere impostazioni di crittografia del nodo diverse, con le seguenti eccezioni. La clonazione del nodo non è supportata da sorgenti con nodi non crittografati a destinazioni con nodi crittografati per le seguenti appliance:
...
** Da un'appliance nodo SG1200 senza crittografia del nodo a un'appliance nodo SG1200 con crittografia del nodo
** Un dispositivo nodo SG110 senza crittografia nodo a un dispositivo SG110 con crittografia nodo
** Da un dispositivo nodo SG120 senza crittografia del nodo a un dispositivo nodo SG120 con crittografia del nodo
...
* Determina se la modalità RAID sull'appliance di destinazione deve essere modificata rispetto all'impostazione predefinita, in modo da poter specificare queste informazioni durante l'installazione iniziale dell'appliance di destinazione prima della clonazione. Puoi visualizzare le informazioni sulla modalità RAID corrente del nodo dell'appliance di origine nella pagina *Nodi* di Grid Manager. Seleziona la scheda *Hardware* per l'appliance.
+
