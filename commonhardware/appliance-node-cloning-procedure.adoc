...
summary: 'Il processo di clonazione potrebbe richiedere diversi giorni per trasferire i dati tra il nodo di origine (appliance da sostituire) e l"appliance di destinazione (nuova).' 
---
= Clonare un nodo appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Se esistono problemi che impediscono la clonazione, *Avvia clonazione* non è abilitata e i problemi da risolvere vengono elencati come *Stato della connessione*. Questi problemi sono elencati nella pagina iniziale di StorageGRID Appliance Installer sia del nodo di origine che dell'appliance di destinazione. Viene visualizzato un solo problema alla volta e lo stato si aggiorna automaticamente al variare delle condizioni. Risolvere tutti i problemi di clonazione per abilitare *Avvia clonazione*.
...
NOTE: Se al termine del processo di cloning riscontri un conflitto di indirizzi IP, assicurati che non siano stati assegnati lease DHCP permanenti a nessuno degli indirizzi MAC delle porte che avevano IP statici sul nodo di origine e che sono state abilitate per il DHCP sul nodo di destinazione. Rimuovi o aggiorna i lease DHCP permanenti sul server DHCP per risolvere questo conflitto.
