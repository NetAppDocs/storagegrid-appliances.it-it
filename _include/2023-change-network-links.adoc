...
NOTE: Le modifiche alla configurazione IP apportate mentre l'appliance è in modalità di manutenzione non vengono applicate all'ambiente StorageGRID installato. Eseguire il collegamento:../maintain/changing-nodes-network-configuration.html[`change-ip` Command] dopo aver riavviato l'appliance in StorageGRID.
