...
---
= Collegare un'appliance StorageGRID o un controller
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
** Da uno a quattro cavi twinax per il collegamento delle quattro porte di rete.
** Per il modello SG120, sono necessari da uno a quattro ricetrasmettitori SFP+ o SFP28 se si intende utilizzare cavi ottici per le porte.
** Per l'SG1200, da uno a quattro ricetrasmettitori QSFP+, QSFP28 o QSFP56 (solo 200GbE NIC) se si prevede di utilizzare cavi ottici per le porte.
...
Connessioni della porta SG120:

image::../media/sg120_connections.png[Connettori posteriori SG120]

Connessioni delle porte SG1200:

image::../media/sg1200_connections.png[Connessioni delle porte SG1200]
...
* 10/25/40/100-GbE con NIC 100G
* 10/25/50/100/200-GbE (solo NIC 200GbE)

 a| 
Connettiti alla rete Grid e alla rete client per StorageGRID. Vedi link:gathering-installation-information-sg120-and-sg1200.html#port-bond-modes["Modalità di collegamento delle porte (SG120 e SG1200)"]
...
[cols="2a,2a"]
|===
| Velocità di collegamento SG120 (GbE) | Attrezzatura necessaria 
...
[cols="2a,2a"]
|===
| Velocità di collegamento SG1200 (GbE) | Attrezzatura necessaria 
...
NOTE: Lo stato del collegamento fisico per la porta 1 non è disponibile nel software e deve essere verificato in questo momento utilizzando il LED di stato sul controller SG5800.
...
.Prima di iniziare
* Solo per SG6200: Il cavo breakout da 100GbE a 4x25GbE fornito con l'appliance serve per collegare i due storage controller al controller SG6200-CN.
* Si dispone di cavi Ethernet RJ-45:
+
** Cavo RJ-45 per il collegamento della porta di gestione.
** Solo per SG6200: fino a quattro cavi Ethernet RJ-45 aggiuntivi per le porte opzionali che si intende utilizzare, inclusa la seconda porta Admin, la porta BMC sull'SG6200-CN e le porte di manutenzione su ciascuno dei due storage controller E4000.


* Per le porte di rete è disponibile una delle seguenti opzioni. Questi elementi non sono forniti con l'apparecchio.
+
** Da uno a quattro cavi twinax per il collegamento delle quattro porte di rete.
** Da uno a otto ricetrasmettitori SFP+, SFP28, QSFP+, QSFP28 o QSFP56 se si prevede di utilizzare cavi ottici per le porte.
...
.A proposito di questa attività
Le seguenti figure mostrano le porte sul retro del SGF6212 e i tre controller presenti nell'appliance SG6200. L'appliance SG6200 include un SG6200-CN compute controller nella parte superiore e due E4000 storage controller nella parte inferiore.

Connessioni SGF6212:

image::../media/sgf6212_connections.png[Connettori posteriori SGF6212]

Connessioni SG6260:

image::../media/sg6200_e4000_connections.png[Collegamenti da SG6200-CN a E4000]
...
2
 a| 
Quattro porte di rete sul controller SG6200-CN
 a| 
* Quattro porte di rete 10/25/40/100-GbE o 200-GbE con SKU NIC 200G opzionale

 a| 
Connettiti alla rete Grid e alla rete client per StorageGRID. Vedi link:gathering-installation-information-sg6200.html#port-bond-modes["Modalità di collegamento delle porte (SG6200)"]
...
Cinque porte di connessione in totale
 a| 
* Una porta 100GbE sul modello SG6200-CN
* Due porte 10 GbE/25GbE GbE su ciascuno storage controller

 a| 
Collegare ciascun storage controller al controller SG6200-CN.
...
. Collegare le due porte di interconnessione di ciascun storage controller alla porta 100GbE del controller SG6200-CN, utilizzando un cavo da 100-GbE QSFP28 a 4x25-GbE SFP28.
. Collegare le porte di rete dell'appliance agli switch di rete appropriati, utilizzando cavi twinax o cavi ottici e ricetrasmettitori.
+
[cols="2a,2a"]
|===
...
. Se si intende utilizzare la rete di amministrazione per StorageGRID, collegare la porta della rete di amministrazione dell'appliance alla rete di amministrazione utilizzando un cavo Ethernet.
. Se l'apparecchio include un shelf di espansione, consultare la link:../sg6200/adding-expansion-shelf-to-deployed-sg6260.html["Istruzioni per l'aggiunta di un shelf di espansione a un SG6200 già installato"] per informazioni sul cablaggio.
