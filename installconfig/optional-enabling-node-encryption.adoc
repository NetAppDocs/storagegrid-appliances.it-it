...
Inoltre, quando si abilita la modalità FIPS dopo aver abilitato la crittografia del nodo, il modulo NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 viene utilizzato per la crittografia dei dati a riposo. Consultare  https://docs.netapp.com/us-en/storagegrid/admin/manage-tls-ssh-policy.html#select-a-security-policy["Seleziona una politica di sicurezza"^] per ulteriori informazioni.
