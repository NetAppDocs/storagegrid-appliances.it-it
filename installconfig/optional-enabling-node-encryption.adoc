...
---
= Abilita la crittografia del nodo o dell'unità per gli appliance StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Inoltre, quando si abilita la modalità FIPS dopo aver abilitato la crittografia dei nodi, il modulo NetApp StorageGRID Kernel Crypto API 6.1.129-1-ntap1-amd64 viene utilizzato per la crittografia dei dati inattivi. Fare riferimento a https://review.docs.netapp.com/us-en/storagegrid_storagegrid-120-dev/admin/manage-tls-ssh-policy.html#select-a-security-policy["Seleziona una politica di sicurezza"^] per maggiori informazioni.
...
CAUTION: Quando un dispositivo viene installato con la crittografia dei nodi abilitata, viene assegnata una chiave temporanea.  I dati presenti sull'appliance non sono protetti finché l'appliance non è connessa al Key Management System (KMS) e non è impostata una chiave di sicurezza KMS.  Fare riferimento al https://docs.netapp.com/us-en/storagegrid/admin/kms-overview-of-kms-and-appliance-configuration.html["Panoramica della configurazione dell'appliance KMS"^] per ulteriori informazioni.
...
La crittografia dell'unità può essere utilizzata per qualsiasi SSD SED installato in un SG100, SG1000, SG110, SG1100, SG120, SG1200, SGF6112, SGF6212, SG6100-CN o SG6200-CN compute node o controller.

* Per le appliance per i servizi, gli SSD sono i dischi root del nodo.
* Nei controller SG6100-CN o SG6200-CN, le unità SSD vengono utilizzate per la memorizzazione nella cache.
* Nei dispositivi SGF6112 o SGF6212, le unità SSD sono i dischi radice del nodo e vengono utilizzate per lo storage primario dei dati degli oggetti.
...
Quando si abilita la crittografia dell'unità per un dispositivo StorageGRID con unità FIPS, la crittografia FIPS fornita dalle unità FIPS viene utilizzata per la crittografia dei dati inattivi.

È possibile abilitare la crittografia dell'unità durante l'installazione iniziale dell'appliance prima di caricare Grid Manager.  È anche possibile abilitare la crittografia dell'unità o modificare la passphrase impostando l'appliance in modalità di manutenzione.
