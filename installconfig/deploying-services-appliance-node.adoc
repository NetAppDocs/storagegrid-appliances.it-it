...
summary: 'È possibile implementare un"appliance di servizi come nodo di amministrazione primario, nodo di amministrazione non primario o nodo gateway. Tutte le appliance per i servizi possono operare contemporaneamente come nodi gateway e nodi amministrativi (primari o non primari).' 
---
= Distribuisci un nodo dell'appliance di servizi StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.. Selezionare il nodo di amministrazione primario per la griglia in cui verrà distribuito questo gateway dell'appliance o il nodo di amministrazione non primario.
.. Fare clic su *Save* (Salva).
.. Attendere che lo stato di connessione del nuovo indirizzo IP diventi pronto.
...
Scopri link:https://docs.netapp.com/us-en/storagegrid/swnodes/navigating-to-grid-manager.html["configurare la griglia e completare l'installazione"^] e link:monitoring-appliance-installation.html["monitoraggio dell'installazione dell'appliance"].
