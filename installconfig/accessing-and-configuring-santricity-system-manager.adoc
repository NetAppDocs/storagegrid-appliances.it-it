...
---
= Configurare SANtricity per gli appliance StorageGRID SG6260, SG6160, SG6000, SG5700 e SG5800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Le seguenti appliance StorageGRID possono accedere a SANtricity System Manager:
...
* Le chiavi di sicurezza dell'unità, necessarie per sbloccare le unità protette
+

NOTE: Devi configurare e avere accesso a SANtricity System Manager se la funzione Drive Security è abilitata per il tuo dispositivo.
...
** Utilizzare Grid Manager e selezionare *Nodi* > `*_appliance Storage Node_*` > *SANtricity System Manager*
...
.SG6060
--
Per l'appliance di archiviazione SG6060, la sicurezza dell'unità può essere abilitata automaticamente sulle unità SSD solo se la gestione delle chiavi è stata configurata prima di installare l'appliance di archiviazione.
...
.. Per lo scaffale del controller E2800, creare una chiave di sicurezza (una stringa di caratteri condivisa dal controller e dalle unità per l'accesso in lettura e scrittura).
+
...
.SG6260
--
L'appliance di storage SG6260 può essere equipaggiata con unità conformi allo standard FIPS sia nel controller di elaborazione SG6200-CN che nello shelf del controller E4000. La crittografia delle unità viene configurata separatamente per le unità SG6200-CN e per le unità E4000.

.. link:optional-enabling-node-encryption.html#drive-encryption["Attivare Drive Encryption"] per SSD SED installati nel nodo di calcolo SG6200-CN.
.. Creare una chiave di protezione (una stringa di caratteri condivisi dal controller e dalle unità per l'accesso in lettura/scrittura).
+
*** È possibile https://docs.netapp.com/us-en/e-series-santricity/sm-settings/create-internal-security-key.html["creare una chiave interna"^] dalla memoria persistente del controller o utilizzare una chiave esterna fornita da un server di gestione delle chiavi.
*** Per utilizzare una chiave esterna fornita da un server di gestione delle chiavi, è necessario prima https://docs.netapp.com/us-en/e-series-santricity/sm-settings/overview-drive-security.html["stabilire l'autenticazione con un server di gestione delle chiavi"^] in Gestione sistema SANtricity.
...
NOTE: Segui questa procedura solo se non hai accesso a SANtricity System Manager tramite StorageGRID Appliance Installer (*Advanced* > *SANtricity System Manager*) o Grid Manager (*Nodes* > *SANtricity System Manager*).
